...
if sys.argv[1] == 'perceptron':
    ...
    x = df.iloc[0:dataset_size, [0,dataset_columns - 2]].values
    x_std = np.copy(x)
    x_std[:, 0] = (x[:, 0] - x[:, 0].mean()) / x[:, 0].std()
    x_std[:, 1] = (x[:, 1] - x[:, 1].mean()) / x[:, 1].std()
    ...
    model = Ovr(df.iloc[0:dataset_size, dataset_columns - 1].values, _eta=0.01, _iter=20)
else:
...
if sys.argv[1] == 'ovr':
    ...
    if sys.argv[1] == 'adaline':
        x_tst = df.iloc[0:dataset_size, [0,dataset_columns - 2]].values
    for i in range(len(x_tst)):
        prediction = model.predict(x_tst[i])
